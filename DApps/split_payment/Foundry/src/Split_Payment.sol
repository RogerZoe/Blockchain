...
contract SplitPayment {
    ...
    modifier validExpense(uint256 _expenseId) {
        require(_expenseId < expenseCounter, "Invalid expense ID");
        _;
    }
    ...
    function createExpense(
        ...
        require(msg.value != 0 ether, "Eth should not be empty");
        require(_splitters.length > 0, "Empty splitters");
    ...
    function payShare(
        uint256 _expenseId
    ) external payable validExpense(_expenseId) {
        ...
        require(msg.value == share, "Incorrect share amount");
    ...
    function getSplitters(
        uint256 _expenseId
    ) external view validExpense(_expenseId) returns (address[] memory) {
    ...
    function getPaymentStatus(
        uint256 _expenseId,
        address _user
    ) external view validExpense(_expenseId) returns (bool) {
        if (_user == expenses[_expenseId].payer) {
            return expenses[_expenseId].hasPaid[_user];
        }

        address[] memory splitters = expenses[_expenseId].splitters;
        for (uint256 i = 0; i < splitters.length; i++) {
            if (splitters[i] == _user) {
                return expenses[_expenseId].hasPaid[_user];
            }
        }

        revert("User not part of expense");
    }
    ...
    function getTotalParticipants(
        uint256 _expenseId
    ) external view validExpense(_expenseId) returns (uint256) {
